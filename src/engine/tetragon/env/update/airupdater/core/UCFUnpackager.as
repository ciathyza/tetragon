					...
					switch (_mUCFParseState)
					{
						...
						case AT_HEADER:
							...
							if (ZIP_LFH_MAGIC != magic)
							{
								...
								{
									fail("Not an AIR file.", Constants.ERROR_UCF_INVALID_AIR_FILE);
								}
							...
							if ((_generalPurposeBitFlags & 0xFFF9) != 0)
							{
								fail("File uses unsupported encryption or streaming features.",
									Constants.ERROR_UCF_INVALID_FLAGS);
							}
							...
							if (_filenameLength == 0)
							{
								fail("One of the files has an empty (zero-length) name.",
									Constants.ERROR_UCF_INVALID_FILENAME);
							}
							// Fall through
						...
						case AT_FILENAME:
							...
							if (_mFileCount == 0 && _path != "mimetype")
							{
								fail("Mimetype must be the first file.",
									Constants.ERROR_UCF_NO_MIMETYPE);
							}
								...
								&& _compressionMethod != COMPRESSION_NONE)
							{
								fail("File " + _path + " uses an illegal compression method "
									+ _compressionMethod, Constants.ERROR_UCF_UNKNOWN_COMPRESSION);
							}
							...
							{
								if (item == ".") fail("Filename " + _path + " contains a component of '.'", Constants.ERROR_UCF_INVALID_FILENAME);
								if (item == "..") fail("Filename " + _path + " contains a component of '..'", Constants.ERROR_UCF_INVALID_FILENAME);
								if (item == "") fail("Filename " + _path + " contains an empty component.", Constants.ERROR_UCF_INVALID_FILENAME);
								return true;
							});
			...
			try
			{
				switch (_mUCFParseState)
				{
					...
					default:
						...
						fail("Truncated or corrupt.", Constants.ERROR_UCF_CORRUPT_AIR);
				}
			}
			catch (err:Error)
			{
