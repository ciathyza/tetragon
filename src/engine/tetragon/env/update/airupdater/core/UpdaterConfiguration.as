...
{
	import tetragon.env.update.airupdater.descriptors.ConfigurationDescriptor;
	...
	public class UpdaterConfiguration
	{
		...
		private var _configDescriptor:ConfigurationDescriptor;
			...
			if (_configFile)
			{
				if (!_configFile.exists)
				{
					throw new Error("Configuration file \"" + _configFile.nativePath
						+ "\" does not exists on disk.", Constants.ERROR_CONFIGURATION_FILE_MISSING);
				}
				var xml:XML = FileUtils.readXMLFromFile(_configFile);
				_configDescriptor = new ConfigurationDescriptor(xml);
		...
		public function get isCheckForUpdateVisible():Boolean
		{
			...
			var config:int = dialogVisConfig(ConfigurationDescriptor.DIALOG_CHECK_FOR_UPDATE);
		...
		public function get isDownloadUpdateVisible():Boolean
		{
			...
			var config:int = dialogVisConfig(ConfigurationDescriptor.DIALOG_DOWNLOAD_UPDATE);
			if (config >= 0) return config == 1;
			return true;
		}
		...
		public function get isDownloadProgressVisible():Boolean
		{
			...
			var config:int = dialogVisConfig(ConfigurationDescriptor.DIALOG_DOWNLOAD_PROGRESS);
			if (config >= 0) return config == 1;
			return true;
		}
		...
		public function get isInstallUpdateVisible():Boolean
		{
			...
			var config:int = dialogVisConfig(ConfigurationDescriptor.DIALOG_INSTALL_UPDATE);
			if (config >= 0) return config == 1;
			return true;
		}
		...
		public function get isFileUpdateVisible():Boolean
		{
			...
			var config:int = dialogVisConfig(ConfigurationDescriptor.DIALOG_FILE_UPDATE);
			if (config >= 0) return config == 1;
			return true;
		}
		...
		public function get isUnexpectedErrorVisible():Boolean
		{
			...
			var config:int = dialogVisConfig(ConfigurationDescriptor.DIALOG_UNEXPECTED_ERROR);
			if (config >= 0) return config == 1;
			return true;
		}
