package tetragon.env.update.airupdater.descriptors
{
	...
	public class UpdateDescriptor
	{
		...
		public function validate():void
		{
			...
			{
				fail("Unknown update descriptor namespace.", Constants.ERROR_UPDATE_UNKNOWN);
			}
			if (_xml.versionNumber != undefined)
			{
				if (version == "")
				{
					fail("Update versionNumber must have a non-empty value.", Constants.ERROR_VERSION_MISSING);
				}
				if (!(/^[0-9]{1,3}(\.[0-9]{1,3}){0,2}$/.test(version)))
				{
					fail("Update versionNumber contains an invalid value.", Constants.ERROR_VERSION_INVALID);
				}
			}
			else
			{
				if (version == "")
				{
					fail("Update version must have a non-empty value.", Constants.ERROR_VERSION_MISSING);
				}
			}
			
			if (url == "")
			{
				fail("Update URL must have a non-empty value.", Constants.ERROR_URL_MISSING);
			}
			
			if (!validateLocalizedText(_xml.description, _defaultNS))
			{
				fail("Illegal values for localized update/description.", Constants.ERROR_DESCRIPTION_INVALID);
			}
		}
		...
		{
			default xml namespace = _defaultNS;
			return UpdateDescriptor.getLocalizedText(_xml.description, _defaultNS);
		}
		...
		{
			HLog.error("UpdateDescriptor: " + message);
			if (throwError)
			{
