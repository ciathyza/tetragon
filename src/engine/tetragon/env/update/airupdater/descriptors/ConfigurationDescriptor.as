package tetragon.env.update.airupdater.descriptors
{
	...
	public class ConfigurationDescriptor
	{
		...
		public function validate():void
		{
			...
			{
				fail("Unknown configuration version.", Constants.ERROR_CONFIGURATION_UNKNOWN);
			}
			if (url == "")
			{
				fail("Configuration URL must have a non-empty value.", Constants.ERROR_URL_MISSING);
			}
			if (!validateInterval(_xml.delay.toString()))
			{
				fail("Illegal value \"" + _xml.delay.toString() + "\" for configuration/delay.",
					Constants.ERROR_DELAY_INVALID);
			}
			if (!validateDefaultUI(_xml.defaultUI))
			{
				fail("Illegal values for configuration/defaultUI.", Constants.ERROR_DEFAULTUI_INVALID);
			}
		}
