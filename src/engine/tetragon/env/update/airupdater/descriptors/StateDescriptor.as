package tetragon.env.update.airupdater.descriptors
{
	...
	public class StateDescriptor
	{
		...
		public function validate():void
		{
			...
			{
				fail("Unknown state version.", Constants.ERROR_STATE_UNKNOWN);
			}
			if (_xml.lastCheck.toString() == "")
			{
				fail("lastCheck must have a non-empty value.", Constants.ERROR_LAST_CHECK_MISSING);
			}
			if (!validateDate(_xml.lastCheck.toString()))
			{
				fail("Invalid date format for state/lastCheck.", Constants.ERROR_LAST_CHECK_INVALID);
			}
			if (_xml.previousVersion.toString() != "" && !validateText(_xml.previousVersion))
			{
				fail("Illegal value for state/previousVersion.", Constants.ERROR_PREV_VERSION_INVALID);
			}
			if (_xml.currentVersion.toString() != "" && !validateText(_xml.currentVersion))
			{
				fail("Illegal value for state/currentVersion.", Constants.ERROR_CURRENT_VERSION_INVALID);
			}
			if (_xml.storage.toString() != "" && (!validateText(_xml.storage) || !validateFile(_xml.storage.toString())))
			{
				fail("Illegal value for state/storage.", Constants.ERROR_STORAGE_INVALID);
			}
			if (["", "true", "false"].indexOf(_xml.updaterLaunched.toString()) == -1 )
			{
				fail("Illegal value \"" + _xml.updaterLaunched.toString() + "\" for state/updaterLaunched.",
					Constants.ERROR_LAUNCHED_INVALID);
			}
			if (!validateFailed(_xml.failed))
			{
				fail("Illegal values for state/failed.", Constants.ERROR_FAILED_INVALID);
			}
			...
			if (count > 0 && count != 3)
			{
				fail("All state/previousVersion, state/currentVersion, state/storage,"
					+ " state/updaterLaunched  must be set.", Constants.ERROR_VERSIONS_INVALID);
			}
		}
