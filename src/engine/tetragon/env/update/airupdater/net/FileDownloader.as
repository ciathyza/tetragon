package tetragon.env.update.airupdater.net
{
	...
	public class FileDownloader extends EventDispatcher
	{
		...
		private function onDownloadResponseStatus(e:HTTPStatusEvent):void
		{
			if (!isHTTPStatusAcceptable(e.status))
			{
				dispatchErrorEvent("Invalid HTTP status code: " + e.status,
					Constants.ERROR_INVALID_HTTP_STATUS, e.status);
			}
		}
		...
		private function onDownloadError(e:ErrorEvent):void
		{
			if (e is IOErrorEvent)
				dispatchErrorEvent(e.text, Constants.ERROR_IO_DOWNLOAD, e.errorID);
			else if (e is SecurityErrorEvent)
				dispatchErrorEvent(e.text, Constants.ERROR_SECURITY, e.errorID);
		}
		...
		private function saveBytes():void
		{
			...
			catch(err1:EOFError)
			{
				...
				dispatchErrorEvent(err1.message, Constants.ERROR_EOF_DOWNLOAD, err1.errorID);
			}
			catch(err2:IOError)
			{
				_isInError = true;
				HLog.warn(toString() + ": IOError - " + err2);
				dispatchErrorEvent(err2.message, Constants.ERROR_IO_FILE, err2.errorID);
			}
		}
			...
			subErrorID:int = 0):void
		{
			_isInError = true;
			if (_urlStream && _urlStream.connected) disposeURLStream();
			dispatchEvent(new DownloadErrorEvent(DownloadErrorEvent.DOWNLOAD_ERROR,
