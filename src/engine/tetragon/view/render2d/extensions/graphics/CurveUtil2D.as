package tetragon.view.render2d.extensions.graphics
{
	public class CurveUtil2D
	{
		...
		public static function quadraticCurve(a1x:Number, a1y:Number, cx:Number, cy:Number,
			a2x:Number, a2y:Number, error:Number = BEZIER_ERROR):Vector.<Number>
		{
			_subSteps = 0;
			_bezierError = error;
		...
		public static function cubicCurve(a1x:Number, a1y:Number, c1x:Number, c1y:Number,c2x:Number,
			c2y:Number, a2x:Number, a2y:Number, error:Number = BEZIER_ERROR):Vector.<Number>
		{
			_subSteps = 0;
			_bezierError = error;
		...
		private static function cubic(t:Number, axis:int):Number
		{
			...
			return (oneMinusT * oneMinusT * oneMinusT * a1) + (3.0 * oneMinusT * oneMinusT * t * c1)
				+ (3.0 * oneMinusT * t * t * c2) + t * t * t * a2;
		}
		 ...
		 */
		private static function subdivide(t0:Number, t1:Number, depth:int, equation:Function,
